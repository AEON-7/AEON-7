...
| [🔮 Aeon Magick Orb](#-aeon-magick-orb) | Pocket Raspberry Pi that gives any AI agent real **eyes + hands** on *any* computer — zero-software KVM-over-IP + on-device Hailo vision, an AI-infra jump box, the Intergalactic Model Share, and the privacy stack. Two image tracks (Pi 4 / Pi 5) |
...
> Real-time speech and vision AI for DGX Spark. Three composable sidecars turn any Spark into a voice agent host: pair two OpenAI-compatible audio endpoints (TTS + ASR) with the Matrix WebRTC bridge to dial your AI directly from any Matrix client — or **video-call it** and let a vision LLM see through your camera — then compose all three into fully-embodied AI **personas** with [create-agentic-personas](https://github.com/AEON-7/create-agentic-personas). **With streaming TTS the agent starts speaking ~1.0 s into a turn on Spark** (~2.1 s for a fully-synthesized non-streaming turn).

| Repo | What it does | ★ |
|---|---|---|
| **[qwen3-tts-server](https://github.com/AEON-7/qwen3-tts-server)** | OpenAI-compatible `/v1/audio/speech` server for Qwen3-TTS-12Hz-1.7B on the [faster-qwen3-tts](https://github.com/andimarafioti/faster-qwen3-tts) CUDA-graph engine — **streams PCM while generating: first audio ~0.4 s, ~1.7× realtime** (measured on Spark). VoiceDesign + voice-clone (`qwen3-tts-clone`) served side-by-side, plus a `GET /v1/audio/voices` cloned-voice library. | ![](https://img.shields.io/github/stars/AEON-7/qwen3-tts-server?style=flat&label=) |
...
The three voice sidecars + the [Qwen3.6-27B AEON Ultimate MTP-XS vLLM main](https://github.com/AEON-7/Qwen3.6-27B-AEON-Ultimate-Uncensored-DFlash) on one Docker bridge = a complete voice agent that starts speaking in about a second. Latency budget (measured, hot path on DGX Spark, streaming TTS via `VOXTRAL_STREAMING=true`):
...
Generation sustains ~1.7× realtime, so playback never underruns. Non-streaming (full-WAV synthesis) reference: TTS for a 1.92 s clip is ~1.48 s, putting the end-to-end voice turn at **~2.1 s**.
...
| Repo | What it does | ★ |
|---|---|---|
| **[vllm-ultimate-dgx-spark](https://github.com/AEON-7/vllm-ultimate-dgx-spark)** | ⭐ **AEON vLLM Ultimate — the current flagship serving image** (`ghcr.io/aeon-7/aeon-vllm-ultimate`). vLLM 0.24.0 + Triton **NVFP4 KV cache** + **TurboQuant K8V4** 4-bit KV compression + native DFlash / EAGLE3 via `--speculative-config` + MaxSafe stability fixes for DGX Spark and RTX 50-series Blackwell. One image serves the entire AEON model catalog. | ![](https://img.shields.io/github/stars/AEON-7/vllm-ultimate-dgx-spark?style=flat&label=) |
...
| | Capability |
|---|---|
| 🖥️ | **Zero-software KVM-over-IP** — see real pixels, move the mouse, type, click on **any** OS, even **pre-OS / BIOS**. HDMI capture in, USB-gadget keyboard/mouse/trackpad out. Nothing installed on the target, ever. |
| 👁️ | **On-device vision** — the Orb's **Hailo AI HAT+** reads the screen: `screen_find` returns click-ready coordinates for any on-screen label, `describe_screen` answers "what is this screen?" — no cloud round-trip. |
| 🛰️ | **AI-infra jump box** — one console over every gateway + DGX Spark: live GPU/CPU/RAM, per-agent token telemetry, container orchestration, and one-click model deploy. |
| 🌌 | **Intergalactic Model Share** — a decentralized IPFS model network. Import from HuggingFace / Ollama / Civitai (SHA-256 verified), share, and pull models across every Orb online — with **ed25519-signed publisher provenance**. |
| 🏁 | **Aeon Bench, from the console** — deploy the [benchmarking pod](https://github.com/AEON-7/Aeon-Bench-Pod) onto a connected GPU server and open its dashboard, without leaving the Orb. |
| 🛡️ | **Privacy stack** — one-click VPN · Tor · I2P · encrypted no-log DNS, and a one-key Tailscale mesh to reach the whole lab from anywhere. |

Two image tracks — **Pi 4 (Bookworm)** and **Pi 5 (Trixie**, with the Hailo super-app + on-device vision/voice/UPS suite**)**. It isn't just Agentic AI — it's **_Robo_-Agentic AI**: the disembodied, given a body.

→ **[aeon-magick-ai-computer-control](https://github.com/AEON-7/aeon-magick-ai-computer-control)** · premade **flashable Pi images** (with a 3D-print enclosure + full build guide **coming soon**) for members on **[Patreon](https://www.patreon.com/AeonForge7/posts/happy-4th-of-162921493)**.
...
| [`vllm-spark-omni-q36`](https://github.com/users/AEON-7/packages/container/package/vllm-spark-omni-q36) | Qwen 3.6 35B-A3B-heretic NVFP4 + DFlash (source-built vLLM + 7 patches) | [Qwen3.6-35B-A3B-heretic-NVFP4-DFlash](https://github.com/AEON-7/Qwen3.6-35B-A3B-heretic-NVFP4-DFlash) |
